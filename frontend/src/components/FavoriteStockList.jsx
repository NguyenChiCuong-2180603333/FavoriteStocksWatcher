...
const getPriceChangeIndicator = (currentPrice, openPrice, previousClosePrice) => {
  ...
  if (currentPrice === null || referencePrice === null || referencePrice === 0) {
    return { color: 'text.secondary', icon: <TrendingFlatIcon fontSize="small" />, changePercent: null };
  }
  const change = currentPrice - referencePrice;
  const changePercent = (change / referencePrice) * 100;
  if (change > 0) {
    return { color: 'success.main', icon: <TrendingUpIcon fontSize="small" sx={{ color: 'success.main' }} />, changePercent };
  } else if (change < 0) {
    return { color: 'error.main', icon: <TrendingDownIcon fontSize="small" sx={{ color: 'error.main' }} />, changePercent };
  } else {
    return { color: 'text.secondary', icon: <TrendingFlatIcon fontSize="small" />, changePercent };
  }
};
...
const FavoriteStockList = ({ stocks, isLoadingExternally, onRemoveStock, onRefreshPrices }) => {
  ...
  const renderSkeletonItem = () => (
    ...
    >
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', minWidth: {xs: '100%', sm: '180px'}, mb: {xs: 1, sm: 0} }}>
        <Skeleton variant="text" width={100} height={24} /> 
      </Box>
      <Box sx={{minWidth: {xs: '100%', sm: '150px'}, textAlign: {xs: 'left', sm: 'right'}, mb: {xs: 1, sm: 0} }}>
        <Skeleton variant="text" width={120} height={18} /> 
        <Skeleton variant="text" width={120} height={18} /> 
      </Box>
      <ListItemSecondaryAction sx={{ position: {xs: 'relative', sm: 'static'}, transform: {xs: 'translateY(0)', sm: 'none'}, right: {xs: 0, sm: 'auto'}, top: {xs:0, sm:'auto'} }}>
  ...
  return (
    <Paper elevation={0} sx={{p: theme.spacing(2)}}> 
        ...
        {typeof onRefreshPrices === 'function' && (
          <Tooltip title="Làm mới giá ngay">
            <span> 
              ...
              >
                {isLoadingExternally && stocks && stocks.length > 0 ? <CircularProgress size={20} color="inherit"/> : <RefreshIcon />}
              </IconButton>
            </span>
          </Tooltip>
        )}
        ...
        {isLoadingExternally && stocks && stocks.length > 0 ? (
          ...
          stocks.map((stock) => {
            if (!stock || !stock.symbol) {
              console.warn("Dữ liệu cổ phiếu không hợp lệ:", stock);
              return null;
            }
            ...
            return (
              ...
              >
                ...
                {!stock.error && stock.currentPrice !== null && (
                  <Box sx={{ display: 'flex', alignItems: 'center', minWidth: {xs: '100%', sm: '180px'}, mb: {xs: 1, sm: 0} }}>
                    <Typography variant="subtitle1" component="span" sx={{ fontWeight: 'medium', mr: 0.5, color: color  }}>
                 ...
                 {!stock.error && stock.currentPrice === null && (
                   <Typography variant="subtitle1" component="span" sx={{ color: 'text.secondary', minWidth: {xs: '100%', sm: '180px'}, mb: {xs: 1, sm: 0} }}>
                      N/A
                    </Typography>
                 )}
